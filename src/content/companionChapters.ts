...
const chapter1: CompanionChapter = {
  ...
  units: [
    ...
    unit("02", "bravais", "Bravais 格子与立方结构", "Bravais lattices", "sc、bcc、fcc 的差别究竟是格子还是基元？", "Kittel 8e, Chapter 1, pp. 6–11", [
      ...
      "以常规立方晶胞边长 a 表示：sc、bcc、fcc 每胞格点数分别为 1、2、4；配位数为 6、8、12；最近邻距为 a、√3a/2、a/√2。fcc 与 hcp 的理想硬球堆积率相同，均约为 0.740，但堆垛序列不同。",
    ], {
    ...
    unit("03", "planes", "晶向与 Miller 指数", "Crystal planes", "怎样用三个整数唯一标记一族平行晶面？", "Kittel 8e, Chapter 1, pp. 11–14", [
      "晶向 [uvw] 表示从一个格点出发、沿 ua₁+va₂+wa₃ 的方向；晶面 (hkl) 则通过截距的倒数定义。操作顺序是：以晶格常数为单位写出三个轴截距，取倒数，再化为最小整数。",
...
const chapter2: CompanionChapter = {
  ...
  units: [
    ...
    unit("02", "reciprocal", "倒格子与 Laue 条件", "Reciprocal lattice", "实空间平移如何变成波矢空间的整数条件？", "Kittel 8e, Chapter 2, pp. 27–33", [
      "定义倒格基矢 bᵢ，使 aᵢ·bⱼ=2πδᵢⱼ。于是任意倒格矢 G=hb₁+kb₂+lb₃ 都满足 exp(iG·T)=1：对所有格子平移，平面波相位完全不变。",
      "入射与出射波矢分别为 k 与 k′，散射矢量 Δk=k′−k。所有晶胞同相叠加的条件是 Δk=G；点乘三条原始矢量便得到三条 Laue 方程。",
    ], {
    ...
    unit("03", "brillouin", "Ewald 球与 Brillouin 区", "Ewald construction", "怎样用一张几何图判断哪些反射可出现？", "Kittel 8e, Chapter 2, pp. 33–38", [
      "弹性散射保持 |k′|=|k|=2π/λ。以入射波矢末端为球心、|k| 为半径作 Ewald 球；当球面穿过倒格点 G 时，便同时满足 Δk=G 与能量守恒。",
    ...
    unit("04", "structure-factor", "结构因子与消光", "Structure factor", "满足 Laue 条件为何仍可能看不到峰？", "Kittel 8e, Chapter 2, pp. 38–44", [
      "Laue 条件只保证不同晶胞同相；同一晶胞内不同基元原子的散射还可能互相抵消。结构因子 S_G=Σⱼ fⱼexp(−iG·rⱼ) 正是这个单胞内的复振幅，峰强与 |S_G|² 成正比。",
...
const chapter4: CompanionChapter = {
  ...
  units: [
    ...
    unit("02", "velocity", "长波极限与群速度", "Group velocity", "声速为什么只由色散曲线在 K=0 的斜率决定？", "Kittel 8e, Chapter 4, pp. 93–95", [
      "波包由相邻 K 的正常模叠加而成，其包络传播速度为 v_g=dω/dK。对单原子链，v_g=a√(C/M)cos(Ka/2)；区中心达到声速，区边界降为零。",
      "当 Ka≪1 时，相邻原子几乎同相，晶格看起来像连续弹性介质。sin(Ka/2)≈Ka/2，于是 ω≈v_s|K|，其中 v_s=a√(C/M)。测得低频声速即可反推出长波有效力常数。",
    ], {
    ...
    unit("03", "diatomic", "双原子链：声学支与光学支", "Diatomic chain", "为什么一个原胞含两个原子就多出一条分支？", "Kittel 8e, Chapter 4, pp. 95–99", [
      "令重复距离为 a，原胞中质量 M₁、M₂ 的两类原子交替排列。每个 K 的运动方程成为 2×2 本征值问题，因此有两个本征频率：低频声学支中两原子在长波极限同相；高频光学支中它们反相，质心近似不动。",
      "区中心声学频率必须为零，因为全体原子同位移只对应刚体平移；光学频率满足 ω²=2C(1/M₁+1/M₂)。区边界两支分别趋于 2C/M₁ 与 2C/M₂，并可能形成频率禁带。",
    ], {
    ...
    unit("04", "quantization", "正常模量子化与声子", "Phonon quantization", "声子为何既有能量又只有准动量？", "Kittel 8e, Chapter 4, pp. 99–102", [
      "在谐近似中，每个 (K,s) 正常模都是独立量子谐振子，能级间隔为 ħω(K,s)。把该模的占据数增加 1，就称为创造一个声子。零点能 ½ħω 不随占据数消失。",
      "声子的 ħK 是 crystal momentum（晶体准动量），不等同于整个晶体的机械动量。晶格只具有离散平移对称，因此散射过程只要求 K 在模倒格矢 G 的意义下守恒。",
    ], {
...
const chapter5: CompanionChapter = {
  ...
  units: [
    ...
    unit("02", "dos", "正常模计数与态密度", "Density of states", "三维声子态密度为什么正比于 ω²？", "Kittel 8e, Chapter 5, pp. 111–116", [
      "周期边界条件让每个 K 态在波矢空间占据体积 (2π)³/V。半径 K 的球内每个极化分支包含 VK³/(6π²) 个状态；若线性色散 ω=vK，则壳层面积 K² 直接转化为 D(ω)∝ω²。",
    ...
    unit("03", "debye", "Debye T³ 定律与 Einstein 模型", "Debye heat capacity", "低温热容为什么普遍呈 T³？", "Kittel 8e, Chapter 5, pp. 116–121", [
      "Debye 模型保留低频声学模的线性色散并用 ω_D 截止。低温时只有 ω≲k_BT/ħ 的长波模被激发；三维态密度 D∝ω²，使可参与的模数随 T³ 增长，因此 C_V∝T³。",
      "Einstein 模型把全部 3N 个模设为同一频率 ω_E，能正确展示量子冻结和高温极限，却因缺少任意低频声学模而在低温给出指数衰减。",
    ], {
    ...
    unit("04", "thermal", "非谐性、热膨胀与热导", "Anharmonicity & conductivity", "声子为什么会散射并产生有限热阻？", "Kittel 8e, Chapter 5, pp. 121–128", [
      ...
      "声子气体的动理学近似给出 κ=(1/3)C_Vvℓ。Normal 过程满足 K₁+K₂=K₃，本身不消除总晶体准动量；Umklapp 过程满足 K₁+K₂=K₃+G，能把准动量折回第一 Brillouin 区，是高温本征热阻的关键。边界、缺陷和同位素还会进一步限制平均自由程。",
    ], {
