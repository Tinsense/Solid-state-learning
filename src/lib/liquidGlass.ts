...
void main() {
  ...
  if (u_hasBackground > 0.5) {
     ...
     */
    float dispersionPx = min(6.0, optics.x * 0.30) * refractField;
    vec2 dispersionUV = bendDir * dispersionPx * pxToUV;

    vec2 uvR = clamp(baseUV + refractionUV * 1.30 + dispersionUV, vec2(0.001), vec2(0.999));
    vec2 uvG = clamp(baseUV + refractionUV,                  vec2(0.001), vec2(0.999));
    vec2 uvB = clamp(baseUV + refractionUV * 0.70 - dispersionUV, vec2(0.001), vec2(0.999));
  ...
  vec3 highlight = vec3(0.98) * fresnel * mix(0.108, 0.042, u_theme);
  highlight += vec3(1.0) * glare * mix(0.43, 0.16, u_theme);
  ...
  float edgeAlpha = fresnel * mix(0.034, 0.014, u_theme) + glare * mix(0.20, 0.085, u_theme);
...
class SharedGlassRenderer {
  ...
  render(elements: HTMLElement[], time: number) {
    ...
    visible.forEach(({ element, rect }, index) => {
      ...
      if (element.matches(".site-header, .chapter-rail, .mobile-rail-toggle")) {
        refractionPx = 4.7;
        refractionRange = 10;
        fresnelRange = 1.05;
        glareRange = 0.64;
